...
export function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  ...
  const handleAnonymousSignIn = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await signInAnonymously(pseudo.trim());
      onSuccess();
      onClose();
    } catch (err) {
      console.error('Anonymous sign-in failed:', err);
      setError('Échec de la connexion anonyme. Veuillez réessayer.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleAnonymousSignIn} className="space-y-3">
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Connexion...' : 'Commencer à jouer'}
            </button>
          </form>
          ...
          <p className="text-xs text-gray-500 text-center">
            En mode invité, vos scores seront sauvegardés localement uniquement
          </p>
        </div>
      </div>
    </div>
